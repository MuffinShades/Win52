...
public:
    ...
    template<class _KeyTy> bool exists(_KeyTy key) {
        if (!this->roots || this->hashSz <= 0) return false;

        u64 hsh = this->computeHash((char*) &key, sizeof(_KeyTy));

        if (hsh > this->hashSz)
            return false; //well shoot

        hash_node<_storeType>* cur = this->roots[hsh];

        if (!cur) return false;

        while (cur) {
            if (cur->sz == sizeof(_KeyTy) && memeq(&key, cur->key, sizeof(_KeyTy)))
                return true;
            cur = cur->prev;
        }
    ...
    bool exists(std::string key) {
        ...
        u64 hsh = this->computeHash(const_cast<char*>(key.c_str()), keyLen);
        ...
        while (cur) {
            if (cur->sz == keyLen && memeq(const_cast<char*>(key.c_str()), cur->key, keyLen))
                return true;
            cur = cur->prev;
        }
